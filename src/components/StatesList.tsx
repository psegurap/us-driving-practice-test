...
export default function StatesList() {
    return (
        <div className="gap-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-16">
            {Object.entries(available_states).map(
                ([slug, state], index: number) => (
                    <div
                        key={state.name}
                        className="bg-white shadow-sm  dark:bg-gray-800/50 dark:shadow-none dark:outline dark:-outline-offset-1 dark:outline-white/10 rounded-sm group relative p-6 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-cyan-600"
                    >
                        <div>
                            <span
